...
# print any given Dictionary displaying each key, followed by a colon
def display_inventory(inventory):
    for supply, amount in inventory.items():  # then a space, then the corresponding value, then a newline
        ...
        print(f'{supply}: {amount}')


# add the contents of the added_items list to the inventory dictionary
def add_to_inventory(inventory, added_items):
    for supply in added_items:  # calling with dictionary and items list results in adding items into dictionary
        # as keys and values are set to 1 , or +1 if existed in inventory before
        inventory[supply] = inventory.get(supply, 0) + 1
        # if same occures more, then dictionary values are incremented those amounts o time


# remove the contents of the removed_items list from the inventory dictionary
def remove_from_inventory(inventory, removed_items):
    for supply in removed_items:
        # decrease by one at every mention -> leaves supplies with 0 volume
        inventory[supply] = inventory.get(supply, 0) - 1
    # Remove less-than-one supplies by key
    [inventory.pop(supply) for supply in [
        amount for amount in inventory if inventory[amount] < 1]]
    # as nothing < 1 stays, will not display deducted items, not in original dict


# PLEASE make this ne prettier, or maybe lets use Pandas and DataFrames?
def print_table(inventory, order=None):
    ...
    max_value_length = max(max(len(str(x))
                               for x in inventory.values()), len(label_2))

    # Sort the inventory depending on which order parameter is passed to function
    if order == 'count,asc':
        sorted_inventory = sorted(
            inventory.items(), key=operator.itemgetter(1))
...
def export_inventory(inventory, filename="export_inventory.csv"):
    export_list = [supply for supply, amount in inventory.items()
                   for count in range(0, amount)]  # creating export list
...
if __name__ == "__main__":
    ...
    non_inventory = {}
    added_items = ['Star Wars', 'ToyStory',
                   'HubalaBaba', 'Seventh', 'Star Wars']
